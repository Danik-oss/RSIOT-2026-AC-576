...
```promql
rate(web23_http_requests_total[5m])
```

#### Доступность (SLO 99.5%)

```promql
(sum(rate(web23_http_requests_total{status!~"5.."}[5m])) / sum(rate(web23_http_requests_total[5m]))) * 100
```

#### p95 latency (SLO ≤200ms)

```promql
histogram_quantile(0.95, sum(rate(web23_http_request_duration_seconds_bucket[5m])) by (le))
```
...
- [ ] monitoring-app деплоится с 2 репликами
 - [ ] Метрики web23_* доступны на /metrics
